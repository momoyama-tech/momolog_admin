...
momologプロジェクトの運営者向け管理画面です。団体の管理・ユーザー管理・動画の横断管理・テーマ管理など、サービス全体の運営作業を担います。
...
| **momolog_admin** | **運営者が団体・ユーザー・動画・テーマを管理する管理画面（このリポジトリ）** |
| momolog_official | 一般ユーザーが団体一覧を閲覧する公開サイト（認証不要・表示のみ） |
...
[団体のYouTubeアカウントをOAuth 2.0で連携（1回のみ）]
    ↓
[動画撮影・加工（Cloud Run: momolog_processor でBGM追加等） → メタデータ入力 → 団体選択]
    ↓
...
[YouTube側で運営が手動で公開設定を変更]  ← adminで動画ステータスを確認
    ↓
[momolog_official が Firestore /groups をリアルタイム取得・表示]
    ↓
[ユーザー（学生・教職員・一般）]
```
...
>
> ⚠️ **momolog_officialの現在のスコープは「団体一覧の表示」のみ**
> officialはFirestoreの `/groups` をリアルタイムリスナーで取得して表示するだけ。
> adminが団体を追加・削除するとofficialに即時反映される。

### インフラ構成

| サービス | 用途 |
|---|---|
| Firebase Authentication | ユーザー・管理者の認証（Google Sign-In） |
| Firebase Firestore | 動画URL・団体・ユーザー・YouTube認証情報・テーマの管理 |
| Firebase Storage | 動画ファイルの一時保管 |
| Firebase Admin SDK | サーバーサイドでのFirestore/Storage操作（systemが使用） |
| YouTube Data API v3 | OAuth 2.0による動画の自動投稿（systemが使用） |
| Cloud Run (momolog_processor) | 動画加工（BGM追加等）（systemが使用） |
| Netlify | momolog_system のホスティング |
...
- **momolog_officialに表示されるUIテーマ（カラー・ロゴ等）の設定・変更**
...
│   │   ├── types.ts              # 型定義（Group / AppUser / Video / Theme）
│   │   └── stores/
...
| `/theme` | テーマ管理 | momolog_officialに表示されるUIテーマの設定・変更 |
...
/themes/{themeId}                     // ⚠️ admin専用コレクション（将来設計予定）
  - （スキーマは詳細設計待ち）
...
>
> ⚠️ **`/themes` コレクションのスキーマは詳細設計待ち**
> テーマ管理機能の実装前に別途スキーマを確定すること。
...
service cloud.firestore {
  match /databases/{database}/documents {
    ...
    // ユーザーコレクション: adminのみ書き込み可、自分自身は読み取り可
    match /users/{userId} {
    ...
    // テーマコレクション: adminのみ読み書き可（officialからは読み取りのみ許可予定）
...
### `src/routes/(admin)/theme/+page.svelte`（テーマ管理）

> ⚠️ `/themes` コレクションのFirestoreスキーマは詳細設計待ち。
> 現在はプレースホルダーとして配置のみ行い、スキーマ確定後に実装を進めること。

```svelte
<script lang="ts">
  // TODO: /themes コレクションのスキーマ確定後に実装
</script>

<h2 class="text-2xl font-bold mb-6">テーマ管理</h2>
<div class="bg-white p-6 rounded-lg shadow-sm">
  <p class="text-gray-500 text-sm">
    momolog_official に表示されるUIテーマ（カラー・ロゴ等）を管理します。<br />
    ⚠️ Firestoreスキーマの詳細設計完了後に実装予定です。
  </p>
</div>
```
...
- **テーマ管理 `/themes`**: スキーマ詳細設計が完了するまで実装を進めないこと
